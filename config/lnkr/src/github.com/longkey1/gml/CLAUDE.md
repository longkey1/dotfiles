...
- **messages.go**:
  - `ListMessages()`: Fetches messages with pagination (auto-paginated, transparent to caller), supports query, labels, field filtering
  - `GetMessage()`: Retrieves a single message by ID with full details
  - `ParseFields()`: Parses comma-separated field strings
  - Body extraction with MIME type handling (text/plain preferred, then text/html)
  - `MessageInfo` and `MessageDetail` structs include `ThreadID` and `URL` fields

- **labels.go**:
  - `LabelIndex`: Fast lookup structure for label names/IDs (name→ID, ID→name, ID passthrough)
  - `FetchLabelIndex()`: Fetches all labels and builds index (case-normalized)
  ...
  - `GetUserEmail()`: Retrieves authenticated user's email address
  - `BuildMailURL()`: Generates Gmail web UI URL for a message
...
- `version.GoVersion`: Go runtime version (auto-detected via `runtime.Version()`)

In development builds, Version/CommitSHA/BuildTime default to "dev" / "unknown".
